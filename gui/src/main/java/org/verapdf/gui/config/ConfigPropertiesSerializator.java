...
public final class ConfigPropertiesSerializator {
    ...
    private static final Logger LOGGER = Logger
            .getLogger(ConfigPropertiesSerializator.class);
     ...
     *
     * @param config
     *            config object for serialization
     * @param path
     *            path for the result file
     * @throws IOException
     *             if I/O errors occurs
     * @throws IllegalArgumentException
     *             if config is null, path is null or path is invalid path for
     *             write a file
     */
    public static void saveConfig(Config config, Path path) throws IOException {
        ...
        if (path == null) {
            throw new IllegalArgumentException("Path can not be null");
                ...
                || (path.toFile().exists() && !path.toFile().canWrite())) {
            throw new IllegalArgumentException(
                    "Path should specify a write accessible file path");
        }

        try (FileWriter writer = new FileWriter(path.toFile())) {

            Properties settings = new Properties();
            settings.setProperty(PROPERTY_MAX_NUMBER_FAILED_CHECKS,
                    String.valueOf(config.getMaxNumberOfFailedChecks()));
            settings.setProperty(
                    PROPERTY_MAX_NUMBER_DISPLAYED_FAILED_CHECKS,
                    String.valueOf(config.getMaxNumberOfDisplayedFailedChecks()));
            settings.setProperty(PROPERTY_SHOW_PASSED_RULES,
                    String.valueOf(config.isShowPassedRules()));
            settings.setProperty(PROPERTY_METADATA_FIXER_PREFIX,
                    config.getMetadataFixerPrefix());
            settings.setProperty(PROPERTY_FIX_METADATA_PATH_FOLDER, config
                    .getFixMetadataPathFolder().toString());
            settings.setProperty(PROPERTY_PROFILES_WIKI_PATH,
    ...
    private static String getStringValue(Properties settings, String key) {
        ...
        if (prefix == null) {
            throw new IllegalArgumentException(
                    "Settings should contain property " + key);
        }
        return prefix;
    }
     ...
     * @throws IllegalArgumentException
     *             if any error with values occurs or some value is missing
     */
    public static Config loadConfig(Path path) throws IOException {
        Properties properties = new Properties();
        if (!path.toFile().isFile() || !path.toFile().canRead()) {
            throw new IllegalArgumentException(
                    "Path should specify an existed and read accessible file");
        }
        ...
        try {
            builder.maxNumberOfFailedChecks(getIntegerValue(properties,
                    PROPERTY_MAX_NUMBER_FAILED_CHECKS));
        } catch (IllegalArgumentException e) {
            LOGGER.error(
                    "Property "
                            + PROPERTY_MAX_NUMBER_FAILED_CHECKS
                            + " is missing or containing a wrong value. Setting it to default",
                    e);
        }
        try {
            builder.maxNumberOfDisplayedFailedChecks(getIntegerValue(
                    properties, PROPERTY_MAX_NUMBER_DISPLAYED_FAILED_CHECKS));
        } catch (IllegalArgumentException e) {
            LOGGER.error(
                    "Property "
                            + PROPERTY_MAX_NUMBER_DISPLAYED_FAILED_CHECKS
        ...
        try {
            builder.showPassedRules(getBooleanValue(properties,
                    PROPERTY_SHOW_PASSED_RULES));
        } catch (IllegalArgumentException e) {
            LOGGER.error(
                    "Property "
                            + PROPERTY_SHOW_PASSED_RULES
        ...
        try {
            builder.metadataFixerPrefix(getStringValue(properties,
                    PROPERTY_METADATA_FIXER_PREFIX));
        } catch (IllegalArgumentException e) {
            LOGGER.error(
                    "Property "
                            + PROPERTY_METADATA_FIXER_PREFIX
        ...
        try {
            builder.fixMetadataPathFolder(getPathValue(properties,
                    PROPERTY_FIX_METADATA_PATH_FOLDER));
        } catch (IllegalArgumentException e) {
            LOGGER.error(
                    "Property "
                            + PROPERTY_FIX_METADATA_PATH_FOLDER
        ...
        try {
            builder.profilesWikiPath(getStringValue(properties,
                    PROPERTY_PROFILES_WIKI_PATH));
        } catch (IllegalArgumentException e) {
            LOGGER.error(
                    "Property "
                            + PROPERTY_PROFILES_WIKI_PATH
